...
export async function dosyaYukle(supabase, dosya, onEk = '') {
  ...
  // contentType, gorselSikistir'in GERÇEKTEN ürettiği tipten (webp
  // desteklenmeyen tarayıcılarda jpeg'e düşebilir) okunur — sabit
  // 'image/webp' yazmak, tarayıcı sessizce farklı bir formata düştüğünde
  // Storage'a yanlış Content-Type ile yüklenmesine yol açardı.
  const { error } = await supabase.storage
    .from('haber-gorselleri')
    .upload(dosyaYolu, yuklenecekDosya, { contentType: yuklenecekDosya.type })
  if (error) {
