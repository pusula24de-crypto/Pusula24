...
// Seçilen dosyayı 1600px'i aşmayacak, ~300-350 KB hedefli, tarayıcının
// GERÇEKTEN encode edebildiği bir formata (tercihen WebP, desteklenmiyorsa
// JPEG) çevirir. Zaten küçük dosyalar da çevrilir ama büyütülmez
// (kütüphane varsayılan olarak yukarı ölçeklemez). HEIC/HEIF girdiler
// önce JPEG'e çevrilir (bkz. heicToJpegDonustur), diğer formatlar
// doğrudan işlenir. Hata durumunda fırlatır.
export async function gorselSikistir(dosya) {
  ...
  const hedefTip = webpDestekli ? 'image/webp' : 'image/jpeg'
  ...
  // Kütüphane/tarayıcı yine de istenenden FARKLI bir tip döndürebilir
  // (ör. beklenmedik bir cihazda jpeg de desteklenmiyorsa PNG'ye düşer) —
  // dosyayı HER ZAMAN blob'un gerçek tipiyle etiketliyoruz, asla varsayılan
  // hedefle değil.
  const gercekTip = sikistirilmis.type || hedefTip
  const uzanti = uzantiVer(gercekTip) || uzantiVer(hedefTip)
