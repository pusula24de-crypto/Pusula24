...
// ============================================================
// GEÇİCİ TEŞHİS MODU — iPhone yüklemelerindeki bozuk/verimsiz sıkıştırma
// sorununun GERÇEK kök nedenini (varsayım değil, kanıt) tespit etmek için.
// Her adımı hem console.log'a yazar HEM DE (uzaktan hata ayıklama kurulumu
// gerektirmesin diye) tek bir alert() penceresinde özetler. İş bitince
// TESHIS_MODU'nu false yap ya da bu bloğu ve aşağıdaki teshisEkle/
// teshisGoster çağrılarını tamamen kaldır.
// ============================================================
const TESHIS_MODU = true
let teshisSatirlari = []

function teshisEkle(satir) {
  if (!TESHIS_MODU) return
  teshisSatirlari.push(satir)
  console.log('[TEŞHİS]', satir)
}

function teshisGoster() {
  if (!TESHIS_MODU || teshisSatirlari.length === 0) return
  if (typeof window !== 'undefined' && typeof window.alert === 'function') {
    window.alert('GÖRSEL YÜKLEME TEŞHİSİ:\n\n' + teshisSatirlari.join('\n'))
  }
  teshisSatirlari = []
}
...
export async function gorselSikistir(dosya) {
  teshisEkle(`Seçilen dosya: type="${dosya.type}" name="${dosya.name}" size=${dosya.size} bayt`)

  try {
    if (!dosya.type?.startsWith('image/') && !heicMi(dosya)) {
      teshisEkle('SONUÇ: reddedildi — image/* tipinde değil ve HEIC/HEIF de değil.')
      throw new Error('Yalnızca görsel dosyaları yüklenebilir (JPG, PNG, WebP vb.).')
    }
    if (dosya.size > MAKS_DOSYA_BOYUTU) {
      teshisEkle('SONUÇ: reddedildi — 15 MB üstü.')
    ...
    const heicTespitEdildi = heicMi(dosya)
    teshisEkle(`HEIC/HEIF olarak algılandı mı: ${heicTespitEdildi}`)

    if (heicTespitEdildi) {
      try {
        islenecekDosya = await heicToJpegDonustur(dosya)
        teshisEkle(
          `HEIC→JPEG dönüşümü BAŞARILI: type="${islenecekDosya.type}" size=${islenecekDosya.size} bayt`
        )
      } catch (err) {
        teshisEkle(`HEIC→JPEG dönüşümü BAŞARISIZ: ${err?.message || err}`)
        throw new Error(
    ...
    try {
      ...
      teshisEkle(`Sıkıştırma sonucu: type="${sikistirilmis.type}" size=${sikistirilmis.size} bayt`)
    } catch (err) {
      teshisEkle(`Sıkıştırma BAŞARISIZ: ${err?.message || err}`)
    ...
    teshisEkle(`SONUÇ: başarılı — çıktı ${webpDosya.size} bayt, tip ${webpDosya.type}`)
    return { dosya: webpDosya, orijinalBoyut, yeniBoyut: webpDosya.size }
  } catch (err) {
    teshisEkle(`FIRLATILAN HATA: ${err?.message || err}`)
    throw err
  } finally {
    teshisGoster()
  }
